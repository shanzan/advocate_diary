...
class CaseOperations
{
    ...
    public function getPreviousDate($caseid){
        $sql = "SELECT previous_date FROM allcases WHERE case_id=:caseid";
        //prepare query
        $statement=$this->con->prepare($sql);
        $statement->bindParam(':caseid',$caseid);
        //query execute or not check
        if ($statement->execute()) {
            ...
            return $result['previous_date'];
        } else {
            return "No Value";
        }
    }
    public function allcasedates($userid){
        $sql = "SELECT DISTINCT DATE(next_date) FROM allcases WHERE user_id=:userid";
//        //prepare query
        $statement=$this->con->prepare($sql);
        $statement->bindParam(':userid',$userid);
        //query execute or not check
