...
class DBOperations{
    ...
    //forget sub exists
    public function Forget_userExixts($email){
        $sql="SELECT * From allusers WHERE user_email='$email'";
        $stm1=$this->con->prepare($sql);
        $stm1->execute();
    ...
    function enterforgetpass($email,$code) {
        $sql="UPDATE allusers SET forgetpass=:code WHERE user_email=:email";
        //prepare query
        $statement=$this->con->prepare($sql);
        //bind paramete with the input fields
    ...
    public function getforgetcode($email){
        $sql = "SELECT forgetpass FROM allusers WHERE user_email=:email";
        //prepare query
        $statement=$this->con->prepare($sql);
        $statement->bindParam(':email',$email);
        //query execute or not check
        if ($statement->execute()) {
            ...
            return $result['forgetpass'];
        } else {
            return "No Value";
        }
    }
    function enterforgetpasssubuser($email,$code) {
        $sql="UPDATE allsubusers SET forgetpass=:code WHERE sub_user_email=:email";
        //prepare query
        $statement=$this->con->prepare($sql);
        //bind paramete with the input fields
        $statement->bindParam(':code',$code);
    ...
    public function getforgetcodesubuser($email){
        $sql = "SELECT forgetpass FROM allsubusers WHERE sub_user_email=:email";
        //prepare query
        $statement=$this->con->prepare($sql);
        $statement->bindParam(':email',$email);
        //query execute or not check
